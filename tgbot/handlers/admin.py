...
async def waiting_approval_user(call: types.CallbackQuery, state: FSMContext):
    ...
    sql_phone = select(Phone).where(Phone.user_id == int(user.id))
    sql_addresses = select(Address).where(Address.user_id == int(user.id))

    async with db_session() as session:
        phones = await session.execute(sql_phone)
        addresses = await session.execute(sql_addresses)
        phone_list: List[Tuple[Phone]] = phones.all()
        addresses_list: List[Tuple[Address]] = addresses.all()
        if phone_list:
